...
function VerifyTick.watchpoint_nse_musTick(addr)
  ...
  if get_bank_at_addr(addr) == g_symbols_ram_bank["nse_musTick"] then
    ...
    VerifyTick.calculate(channel_idx)
...
local function macro_prefix(chan_idx)
  return "wMacro@" .. CHANNEL_NAMES[chan_idx + 1] .. "_"
end

function VerifyTick.calculateVolume(chan_idx)
  ...
  -- macro volume
  local macro_vol, macro_vol_idx = read_byte_from_macro(macro_prefix(chan_idx) .. "Vol")
  
  if macro_vol == nil then
    macro_vol = 0xf
  else
    -- select nibble based on parity
    macro_vol = nibble_from_parity(macro_vol, not calc.parity)
  end
...
function VerifyTick.calculateDuty(chan_idx)
  local calc = g_calcs[chan_idx + 1]

  local macro_duty, macro_duty_counter = read_byte_from_macro(macro_prefix(chan_idx) .. "Duty")
  if macro_duty == nil then
    -- read directly-stored duty cycle value
    macro_duty = ram_read_byte_by_name(macro_prefix(chan_idx) .. "Duty", 2)
    calc.is_base_duty = true
  else
    macro_duty = nibble_from_parity(macro_duty, not calc.parity)
...
function VerifyTick.calculateNote(chan_idx)
  local calc = g_calcs[chan_idx + 1]
  calc.base_note = ram_read_byte_by_name("wMusChannel_BasePitch", chan_idx)
  if calc.portrate ~= 0 then
    -- portamento and arp are mutually exclusive
    calc.note = calc.base_note
    calc.arptype = "portamento"
  else
    -- standard arpeggio
    local macro_arp, macro_arp_counter = read_byte_from_macro(macro_prefix(chan_idx) .. "Arp")
    -- fixed macros are stored at odd addresses
    local macro_fixed = ram_read_word_by_name(macro_prefix(chan_idx) .. "Arp") % 2 == 1
    ...
    if macro_fixed then
      ...
      calc.note = math.max(calc.base_note + macro_offset, 0)
      calc.arptype = "relative=" .. HX(macro_arp)
    end
  end
end

function VerifyTick.calculateFrequency(chan_idx)
  local calc = g_calcs[chan_idx + 1]
  ...
  -- detune macro
  local detune_macro, detune_macro_counter = read_byte_from_macro(macro_prefix(chan_idx) .. "Detune")
  ...
  if detune_macro ~= nil then 
    ...
    calc.detune_macro_base_value = read_byte_from_macro(macro_prefix(chan_idx) .. "Detune", -3) - 0x80
  else
    calc.detune_macro_base_value = 0
  end

  -- vibrato
  local vib_macro, vib_macro_idx = read_byte_from_macro(macro_prefix(chan_idx) .. "Vib")
  local vib_offset = 0
  ...
  if calc.portrate ~= 0 then
    ...
    local prev_port_freq = ram_read_word_by_name(macro_prefix(chan_idx) .. "Arp")
    calc.prev_port_freq = prev_port_freq
    local new_port_freq = prev_port_freq
    if math.abs(prev_port_freq - note_freq) <= calc.portrate then
      -- jump straight to target frequency, since we're close enough
      new_port_freq = note_freq
    else
      -- add to frequency instead.
      new_port_freq = prev_port_freq + sign(note_freq - prev_port_freq) * calc.portrate
    end
...
-- calculates what the channel should output after the next update
-- (this will be compared to the actual output later for verification.)
function VerifyTick.calculate(chan_idx)
  g_calcs[chan_idx + 1] = {}
  local calc = g_calcs[chan_idx + 1]

  -- PARITY ------------------------------------------------------------------
  local channel_bit = bit.lshift(1, chan_idx)
  local previous_parity = bit.band(channel_bit, ram_read_byte_by_name("wMusChannel_ReadNibble")) ~= 0

  -- parity flips.
  calc.parity = not previous_parity
  
  if channel_is_square(chan_idx) then
    VerifyTick.calculateVolume(chan_idx)
    VerifyTick.calculateDuty(chan_idx)
    calc.portrate = ram_read_byte_by_name("wMusChannel_portrate", chan_idx)
    VerifyTick.calculateNote(chan_idx)
    VerifyTick.calculateFrequency(chan_idx)
  end

  if channel_is_triangle(chan_idx) then
    calc.portrate = ram_read_byte_by_name("wMusChannel_portrate", chan_idx)
    VerifyTick.calculateNote(chan_idx)
    VerifyTick.calculateFrequency(chan_idx)
  end
...
function VerifyTick.compare(chan_idx)
  ...
  if chan_idx == CHAN_DPCM or chan_idx == CHAN_NOISE then
    -- TODO: verify DPCM channels and channel noise
    return
  end
